...
noteContinue.addEventListener("click", () => {
    ...
    if (step === 6) {
        ...
        // For clues, hide note to show background
        note.classList.add("hidden");
...
function showNote() {
    noteText.textContent = notes[step-1]?.text || notes[step].text;
...
function handleClick(item) {
    if (step === 3 && item === "mat") {
        step = 4;
        showNote();
    } else if (step === 4 && item === "db") {
        step = 5;
        showNote();
    } else if (step === 5 && item === "brownie") {
        step = 6;
        showBadge();
    }
    // wrong clicks do nothing
}
